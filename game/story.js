...
var time_til_spawn = 3500;  //Random time between 2 and 5 seconds.
...
var mapa_time;
	...
	function preload(){
		...
		game.load.image('level1', 'assets/village.png',800,600);
		game.load.image('level2', 'assets/beach.png',800,600);
		game.load.image('level3', 'assets/river.png',800,600);
		game.load.image('level4', 'assets/forest.png',800,600);
		game.load.image('level5', 'assets/mountain.png',800,600);
		game.load.image('level6', 'assets/mountain2.png',800,600);
		game.load.image('level7', 'assets/clouds.png',800,600);
		game.load.image('level8', 'assets/castle.png',800,600);

		game.load.image('credits','assets/credits.png',800,600);
		...
		game.load.audio('som', ['assets/Castlevania II Simons Quest - Bloody Tears.mp3', 'assets/Castlevania II Simons Quest - Bloody Tears.ogg']);
	...
	function create(){
		...
		lifeText = this.add.text(670, 16, 'Lives: 3', { fontSize: '32px', fill: '#000' });
...
//	this.physics.arcade.overlap(circle,rl,RLdestroy, null, this);
	...
	function level_3(){
		...
		else if(random >= 23 && random < 46 ){
			blueball();
		}
		else if(random >= 46 && random < 69 ){
			greenball();
		}
		else if(random >= 69 && random < 92 ){
			yellowball();
		}
	...
	function level_5(){
		...
		else if(random >= 44 && random < 66 ){
			greenball();
		}
		else if(random >= 66 && random < 88 ){
			yellowball();
		}
		else if(random >= 88 && random < 93 ){
			coffeeball();
		}
		else if(random >=93 && random < 97){
			hearthball();
		}
		else if(random >=97){
			featherball();
	...
	current_time = game.time.time;
	if((level == 1) && (score == 180)){
		level = 11;
		lifesaved = life;
		mapa = game.add.sprite(0,0,'1');
		mapa_time = current_time + 2000;
		speed = 130;
		time_til_spawn = 3500;

	}
	else if((level == 11) && current_time > mapa_time){
		mapa.loadTexture('2');
		level = 111;
		mapa_time = current_time + 1000;
	}
	else if((level == 111) && current_time > mapa_time){
		level = 2;
		mapa.destroy();
		score = 0;
		scoreText.setText('Score: ' + score);
		life = lifesaved;
		lifeText.setText('Lives: ' + life);
		game.background.loadTexture('level2');
	} 
	...
	else if((level == 22) && current_time > mapa_time){
		mapa.loadTexture('3');
		level = 222;
		mapa_time = current_time + 2000;
	}
	else if((level == 222) && current_time > mapa_time){
		level = 3;
		mapa.destroy();
		score = 0;
		scoreText.setText('Score: ' + score);
		life = lifesaved;
		lifeText.setText('Lives: ' + life);
		game.background.loadTexture('level3');
	} 
	...
	else if((level == 33) && current_time > mapa_time){
		mapa.loadTexture('4');
		level = 333;
		mapa_time = current_time + 2000;
	}
	else if((level == 333) && current_time > mapa_time){
		level = 4;
		mapa.destroy();
		score = 0;
		scoreText.setText('Score: ' + score);
		life = lifesaved;
		lifeText.setText('Lives: ' + life);
		game.background.loadTexture('level4');
	} 



	else if((level == 4) && (score == 300)){
		level = 44;
		mapa = game.add.sprite(0,0,'4');
		mapa_time = current_time + 2000;
		lifesaved = life;
		speed = 180;
		time_til_spawn = 3500;
	...
	else if((level == 44) && current_time > mapa_time){
		mapa.loadTexture('5');
		level = 444;
		mapa_time = current_time + 2000;
	}
	else if((level == 444) && current_time > mapa_time){
		level = 5;
		mapa.destroy();
		score = 0;
		scoreText.setText('Score: ' + score);
		life = lifesaved;
		lifeText.setText('Lives: ' + life);
		game.background.loadTexture('level5');
	} 


	else if((level == 5) && (score == 350)){
		level = 55;
		mapa = game.add.sprite(0,0,'5');
		mapa_time = current_time + 2000;
		lifesaved = life;
		speed = 200;
		time_til_spawn = 3400;
	...
	else if((level == 55) && current_time > mapa_time){
		mapa.loadTexture('6');
		level = 555;
		mapa_time = current_time + 2000;
	}
	else if((level == 555) && current_time > mapa_time){
		level = 6;
		mapa.destroy();
		score = 0;
		scoreText.setText('Score: ' + score);
		life = lifesaved;
		lifeText.setText('Lives: ' + life);
		game.background.loadTexture('level6');
	}
	

	else if((level == 6) && (score == 400)){
		level = 66;
		mapa = game.add.sprite(0,0,'6');
		mapa_time = current_time + 2000;
		lifesaved = life;
		speed = 220;
		time_til_spawn = 3200;
	...
	else if((level == 66) && current_time > mapa_time){
		mapa.loadTexture('7');
		level = 666;
		mapa_time = current_time + 2000;
	}
	else if((level == 666) && current_time > mapa_time){
		level = 7;
		mapa.destroy();
		score = 0;
		scoreText.setText('Score: ' + score);
		...
		game.background.loadTexture('level7');
	} 


	else if((level == 7) && (score == 450)){
		level = 77;
		mapa = game.add.sprite(0,0,'7');
		mapa_time = current_time + 2000;
		lifesaved = life;
		speed = 250;
		time_til_spawn = 3000;
	...
	else if((level == 77) && current_time > mapa_time){
		mapa.loadTexture('8');
		level = 777;
		mapa_time = current_time + 2000;

	}
	else if((level == 777) && current_time > mapa_time){
		level = 8;
		mapa.destroy();
		score = 0;
		scoreText.setText('Score: ' + score);
		game.background.loadTexture('level8');
		life = lifesaved;
		lifeText.setText('Lives: ' + life);

	} 
	else if((level == 8) && (score == 500)){
		level = 9;
		circle.destroy();
		game.background.destroy();
		mapa = game.add.sprite(game.world.centerX,900,'credits');
		mapa.anchor.setTo(0.5);
		game.physics.arcade.enable(mapa);
		mapa.enableBody = true;
		mapa.body.velocity.y = 0-speed;
		mapa_time = current_time + 2000;
